...
> 打包/发布前必须先阅读 `docs/打包前必读.md`；Codex 代打包也必须执行其中 Python 3.12 环境闸门，禁止用系统 `python3` 或旧 `.venv` 直接打包。
...
所有赛事、PB、里程碑、记忆都必须能回跳到原始 Activity Detail。
...
- 前端不得根据标题、距离、配速自行推断赛事、PB、里程碑或力量训练总重量。
- 不再设置独立「荣誉」二级页面；赛事成绩、PB、里程碑分别在赛事档案、记录中心和时间轴 / 总览中体现。
...
> 2026-07-13 契约更新：通用 MemoryItem 产品语义已退役。现行产品只保留“赛事 Activity Detail 管理照片、赛事相册只读浏览、Overview Banner 复用赛事首图”的链路。底层 `career_memory_items` 仅作为赛事照片的内部存储实现，不是 AI Snapshot 输入，也不进入年度 Snapshot。
...
| race_id | UUID | 关联赛事 |
| activity_id | UUID | 关联 Activity |
| type | TEXT | 仅允许现行赛事照片链路使用的 photo 类型 |
| metadata | JSON | 排序、角色和受控衍生图逻辑引用；不得向 AI 或前端返回本地路径 |

## 4.5 CareerSnapshot

### 用途

给全生涯 AI fallback 消费的结构化上下文。Career Snapshot 与后续年度 Year Snapshot 必须分离：全生涯 Snapshot 使用 `snapshot_type='career'` 与 `snapshot_version='acs.v1'`，年度 Snapshot 使用独立 scope、年份、版本、指纹和 API，不得复用全生涯 Snapshot ID 或缓存键。
...
不得包含赛事照片内部存储、旧通用记忆、原始 FIT、points、track_json、SQLite schema、本地路径、媒体引用、token 或 Provider 授权信息。
...
- 赛事、PB、记忆、足迹作为下钻页面或二级入口；里程碑作为时间轴 / 总览事件类型，不单独设置「荣誉」标签页
- 移动端采用单列流式布局
