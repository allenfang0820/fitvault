...
| FR-TC-006 | 复盘 Tab 顶部 | 不再显示「🔍 运动复盘 / 正在加载复盘数据」占位头部；复盘内容直接从「本次复盘概览」摘要带开始，AI 状态仅保留为「AI 待开放」状态标记 | ☐ |
| FR-TC-007 | 复盘 Tab 主体 | 显示 8 个 metric cards(心率漂移/后程效率变化/能量断档风险/崩溃事件/运动效率/耐久/步频稳定/训练负荷) | ☐ |
...
| 用例 ID | 操作 | 预期结果 | 通过 |
|---|---|---|---|
| FR-TC-009V | 查看核心状态区 | 心率漂移、后程效率变化、能量断档风险、崩溃事件 4 张卡存在且层级清晰, 标题旁有 hover 说明 | ☐ |
...
| FR-TC-009BV | 对照设计稿右侧模块 | 关键摘要、状态下滑线索、基准核心因素、生理冲击点顺延到 P7.16 补齐 | ☐ |
...
| 四个问题 | 后续任务必须覆盖身体状态变化、失衡原因、事件点、状态路段 | ☐ |
...
| 阶段来源 | 状态阶段只调用 `_renderFatigueReviewStageOverview(data.fatigue_zones || [], data.sport_type, reviewTotalDistanceKm)`, 其中 `sport_type` 仅用于文案分组, `reviewTotalDistanceKm` 仅用于展示语义 | ☐ |
...
| 全程型区间 | 对 2026-05-21 这类覆盖大部分全程且早期开启的区间,标题显示 `整体偏吃力`,不显示为 `开始吃力` | ☐ |
| 全程型事件 | 当存在全程型区间时,关键事件说明使用 `系统识别到的参考点` 类弱锚点文案,仍保留事件公里数 | ☐ |
| 碎片区间摘要化 | 多个碎片化 `fatigue_zones` 在右侧状态区间中汇总为 1-3 条摘要,可显示 `多段波动` 和 `由 N 个状态片段合并` | ☐ |
| 原始图层保留 | 右侧摘要不改变主图疲劳带和事件线,图表仍按后端原始 `fatigue_zones / collapse_events` 显示 | ☐ |
| 阶段条说明 | 阶段条边界说明明确“原始片段”和“右侧状态区间已合并为阅读摘要”的关系 | ☐ |
| 阶段条 Hover | 鼠标悬停阶段条片段时出现解释,说明小片段是回看线索,不代表身体状态在该公里点突然变化 | ☐ |
| 阶段说明降噪 | 用户不应看到 `暂无阶段说明`；阶段条 tooltip 和卡片文案均为用户化解释 | ☐ |
...
| 状态下滑线索 | 事件面板语义为状态下滑线索,事实仍只读 `collapse_events.trigger_km / title / label / type / description`, `sport_type` 仅用于文案分组 | ☐ |
| 事件点说明 | “状态下滑线索”边界说明明确事件是参考点,不代表身体状态在该公里点突然变化 | ☐ |
| 状态区间说明 | “状态区间”边界说明明确区间是持续状态路段,摘要用于理解分布 | ☐ |
...
| 样例 | 检查内容 | 通过 |
|---|---|---|
| 2026-05-21 跑步 | 状态区间显示 `整体偏吃力`; 状态下滑线索使用 `系统识别到的参考点`; 阶段条 hover 不暗示某公里点突然变化; 关系说明显示“事件是点,区间是段” | ☐ |
| 多碎片活动 | 右侧状态区间只显示 1-3 条摘要,可见 `多段波动` 或 `由 N 个状态片段合并`; 阶段条仍保留原始片段并说明右侧已合并为摘要 | ☐ |
| 徒步/登山 | 文案强调爬升、补给、停歇; 不把功率作为核心解释,不出现跑步式强配速解释 | ☐ |
| 骑行 | 文案强调心率、坡度、功率/输出; 不出现跑步式 `配速、恢复段` 作为核心解释 | ☐ |
| 跑步 | 文案可强调配速、心率、恢复段; 不出现骑行式功率输出作为核心解释 | ☐ |
| 无事件/无区间 | 关系说明显示 `本次暂无明显事件点或状态路段`; 空态不显得像异常错误 | ☐ |
| 旧文案扫描 | 用户可见 UI 不出现 `崩溃触发因素 / 触发因素 / Bonk 风险 / 解耦率 / 暂无阶段说明 / 从这里开始 / 这里之后` | ☐ |
| 图层保真 | 主图事件线和疲劳带仍按后端 `collapse_events / fatigue_zones` 原始数据展示 | ☐ |
