...
- 缓存不存在时自动生成；只有显式 --refresh 时才强制刷新活动缓存。
"""
import argparse
import json
import os
...
def write_activity_cache(activities: list[dict], region: str, display_name: str | None) -> None:
...
def load_activity_cache() -> list[dict] | None:
...
def ensure_activity_cache(client, region: str, display_name: str | None, args: argparse.Namespace) -> list[dict]:
    activities = None if args.refresh else load_activity_cache()
    if activities is not None:
        debug(f"使用活动缓存: {len(activities)} 条", args.debug)
        return activities
    if args.no_cache_refresh:
        debug("活动缓存不存在且 --no-cache-refresh 已启用", args.debug)
        return []
    debug("刷新活动缓存", args.debug)
