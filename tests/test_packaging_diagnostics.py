...
class TestPackagingDiagnostics(unittest.TestCase):
    SDK_VERSION = diag.GARMIN_FIT_SDK_EXPECTED_VERSION
    ...
    def test_requirements_and_constraints_lock_garmin_contract(self):
        ...
        self.assertIn(f"garmin-fit-sdk=={self.SDK_VERSION}", requirements)
        self.assertIn("curl_cffi>=0.6", requirements)
        self.assertIn("garminconnect==0.3.6", constraints)
        self.assertIn(f"garmin-fit-sdk=={self.SDK_VERSION}", constraints)
    ...
    def test_garmin_dependency_check_accepts_036_api(self):
        ...
        result = diag.check_garmin_dependencies(
            version_lookup=self._version_lookup(
                {"garminconnect": "0.3.6", "curl_cffi": "0.6.1", "garmin-fit-sdk": self.SDK_VERSION}
            ),
        ...
        self.assertEqual(result["garmin-fit-sdk"], self.SDK_VERSION)
    ...
    def test_garmin_dependency_check_fails_old_version(self):
        with self.assertRaisesRegex(diag.PackagingCheckError, "仅兼容 garminconnect 0.3.6"):
            diag.check_garmin_dependencies(
                version_lookup=self._version_lookup(
                    {"garminconnect": "0.2.8", "curl_cffi": "0.6.1", "garmin-fit-sdk": self.SDK_VERSION}
                ),
    ...
    def test_packaging_prerequisites_runs_python312_runtime_gate(self):
        with TemporaryDirectory() as temp:
            ...
            with mock.patch.object(
                ...
                return_value={"garminconnect": "0.3.6", "curl_cffi": "0.6.1", "garmin-fit-sdk": self.SDK_VERSION},
            ):
        ...
        self.assertEqual(result["packages"]["garmin-fit-sdk"], self.SDK_VERSION)
    ...
    def test_manifest_contains_contract_fields_and_redacts_sensitive_values(self):
        with TemporaryDirectory() as temp:
            ...
            def fake_version(name):
                return {
                    "garminconnect": "0.3.6",
                    "garmin-fit-sdk": self.SDK_VERSION,
                    "curl_cffi": "0.6.1",
                    "requests": "2.32.0",
                    "urllib3": "2.2.0",
                    "certifi": "2026.1.1",
        ...
        self.assertEqual(manifest["packages"]["garmin-fit-sdk"], self.SDK_VERSION)
        self.assertEqual(manifest["compatibility"]["garmin_fit_sdk_expected"], self.SDK_VERSION)
