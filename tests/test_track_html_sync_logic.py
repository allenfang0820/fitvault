...
class TestTrackHtmlSyncLogic(unittest.TestCase):
    ...
    def test_career_tab_starts_derived_refresh_without_blocking_overview(self):
        ...
        upgrade_body = extract_function_body(self.source, "function refreshDerivedMetricsUpgradeInBackground()")
        status_body = extract_function_body(self.source, "function renderDerivedMetricsUpgradeStatus(status)")
        ...
        self.assertIn("get_derived_metrics_upgrade_status", upgrade_body)
        self.assertIn("start_derived_metrics_upgrade", upgrade_body)
        self.assertIn("正在更新运动能力与记录数据，完成后将自动刷新", status_body)
        self.assertIn("refreshAfterDerivedMetricsUpgrade(status)", upgrade_body)
        self.assertIn("renderPerformanceRadar()", extract_function_body(self.source, "function refreshAfterDerivedMetricsUpgrade(status)"))
        self.assertIn("refreshDerivedMetricsUpgradeInBackground()", load_body)
        self.assertIn("refreshCareerDerivedEventsInBackground()", load_body)
        self.assertLess(load_body.find("refreshDerivedMetricsUpgradeInBackground()"), load_body.find("loadCareerFirstPaint()"))
        self.assertLess(load_body.find("refreshCareerDerivedEventsInBackground()"), load_body.find("loadCareerFirstPaint()"))
        self.assertNotIn("await refreshDerivedMetricsUpgradeInBackground()", load_body)
