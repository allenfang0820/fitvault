...
class TestCareerOverviewFrontendRender(unittest.TestCase):
    ...
    def test_banner_carousel_uses_backend_safe_slides(self):
        ...
        self.assertIn("slice(0, 10)", normalize_body)
        self.assertNotIn("slide.media.hasPhoto && slide.media.imageRef", normalize_body)
        self.assertIn("eventType", normalize_body)
        self.assertIn("highlightReason", normalize_body)
        ...
        self.assertIn("activeHero.highlightReason", render_body)
        self.assertIn("safeHtml(badge)", render_body)
