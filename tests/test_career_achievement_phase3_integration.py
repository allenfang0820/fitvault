...
class TestCareerAchievementPhase3Integration(unittest.TestCase):
    def test_resolver_api_timeline_and_overview_chain_is_consistent(self):
        ...
        try:
            ...
            self.assertGreaterEqual(len(achievement_nodes), len(achievements))
            self.assertIn("first_city", {node["subtype"] for node in achievement_nodes})
            self.assertIn("longest_running", {node["subtype"] for node in achievement_nodes})
            self.assertIn("max_ascent", {node["subtype"] for node in achievement_nodes})
            self.assertIn("first_activity", {node["subtype"] for node in achievement_nodes})
            self.assertTrue(all(node["event_type"] and node["highlight_reason"] for node in achievement_nodes))
    ...
    def test_year_filter_sorting_min_score_and_overview_representatives_are_consistent(self):
        ...
        try:
            ...
            self.assertIn("first_city", {node["subtype"] for node in year_nodes})
            self.assertIn("longest_running", {node["subtype"] for node in year_nodes})
            self.assertIn("max_ascent", {node["subtype"] for node in year_nodes})
